...
currentContent=`curl -s -u ${confluenceUser}:${confluencePass} -X GET https://blackiron629.atlassian.net/wiki/rest/api/content/33118?expand=body.storage | python -c "import sys, json; print json.load(sys.stdin)['body']['storage']['value']"`
...
pageVersion=`curl -s -u ${confluenceUser}:${confluencePass} -X GET https://alm-confluence.systems.uk.hsbc/confluence/rest/api/content/1396984912?expand=version | python -c "import sys, json; print json.load(sys.stdin)['version']['number']"+1`
